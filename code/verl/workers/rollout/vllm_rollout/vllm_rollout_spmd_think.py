...
class vLLMMultiTurnViaChatRollout_think(BaseRollout):
    ...
    def extract_content(self,content):
        ...
        if "You:" in extracted_content:
            extracted_content = extracted_content.split("You:")[-1].strip()
        elif "you:" in extracted_content:
            extracted_content = extracted_content.split("you:")[-1].strip()
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        for i_batch in range(batch_size):
            ...
            n_tokens_prefix=self.get_n_tokens(raw_prompt_i_batch,add_generation_prompt=True)# Calculate token count

            for _ in range(n):
                messagess.append(list(raw_prompt_i_batch))  # Ensure it is a list
        ...
        while True:
            ...
            for i_batch_sample in todo:
                messages_=[{"role":msg["role"],"content":msg["content"]} for msg in messagess[i_batch_sample]]# raw_prompt's role is user
            ...
            for i_batch_sample, env_response in zip(todo_, env_response_batched):
                ...
                if dialogue_turns[original_idx] > self.config.environment.max_turns:
                    ...
                    continue
                if any(w in env_response["content"].lower() for w in ["goodbye", "bye", "farewell", "再见", "拜拜", "告辞", "结束对话"]) or player_simulators[i_batch_sample].emo_point<=0:
                    todo.remove(i_batch_sample)
                    continue
            ...
            for i, i_batch_sample in enumerate(todo):
                ...
                if len(prompt_with_chat_template) <= self.config.prompt_length:
                    ...
                    print(f"Error: prompt text too long, length is {len(prompt_with_chat_template)}, max allowed is {self.config.prompt_length}")
        ...
        for i in range(len(player_simulators)):
            snapshot = player_simulators[i].get_analysis_snapshot()
            emo_point_list.append(player_simulators[i].emo_point)
            termination_reasons.append(snapshot["termination_reason"])
            emotion_traces.append(snapshot["emotion_trace"])
            strategy_tags.append(snapshot["strategy_tags"])
        ...
        expanded_termination_reasons = []
        expanded_emotion_traces = []
        expanded_strategy_tags = []
        ...
        for i in range(batch_size*n):
            for _ in range(dialogue_turns[i]): 
                ...
                expanded_termination_reasons.append(termination_reasons[i])
                expanded_emotion_traces.append(emotion_traces[i])
                expanded_strategy_tags.append(strategy_tags[i])
        ...
        non_tensor_batch = {
            ...
            'termination_reason': to_1d_np_array(expanded_termination_reasons),
            'emotion_trace': to_1d_np_array(expanded_emotion_traces),
            'strategy_tags': to_1d_np_array(expanded_strategy_tags),
        }
        # free vllm cache engine
        if vllm_version in ('0.3.1', '0.4.2', '0.5.4', '0.6.3') and self.config.free_cache_engine:
            self.inference_engine.free_cache_engine()
