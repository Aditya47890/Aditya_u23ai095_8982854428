...
class vLLMMultiTurnViaChatRollout(BaseRollout):
    ...
    def tokenize_with_assistant_mask(self,messages):
        ...
        for i_last_message in range(n_messages):
            # Determine who the next message belongs to
            ...
            # Calculate the number of tokens up to the current message
    ...
    def generate_sequences(self, prompts: DataProto,**kwargs)->DataProto:#, environment, **kwargs) -> DataProto: #see verl/single_controller/base/decorator l.54, we can't send these classes as usual.
        ...
        while True:
            ...
            for i, i_batch_sample in enumerate(todo):
                ...
                env_response_message = player_simulators[i_batch_sample].reply(extracted_content)
            ...
            for i_batch_sample,env_response in zip(todo_,env_response_batched):
                ...
                dialogue_turns[i_batch_sample] += 1

                if dialogue_turns[i_batch_sample] > self.config.environment.max_turns:
        ...
        batch = TensorDict(
            {
                ...
                "generation_mask":generation_mask.contiguous()# Return training mask
            },
            batch_size=batch_size)
        non_tensor_batch = {
            'messages': to_1d_np_array(messagess),
            'emo_point': to_1d_np_array(emo_point_list),
            'dialogue_turns': to_1d_np_array(dialogue_turns),
            'termination_reason': to_1d_np_array(termination_reasons),
            'emotion_trace': to_1d_np_array(emotion_traces),
            'strategy_tags': to_1d_np_array(strategy_tags),
        }
