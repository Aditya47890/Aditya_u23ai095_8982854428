...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def preprocess_data(self, data: DataProto) -> DataProto:
        ...
        if data.non_tensor_batch is not None and len(data.non_tensor_batch) > 0:
            print(f"data.non_tensor_batch: {data.non_tensor_batch}")
            ...
            for key, val in data.non_tensor_batch.items():
                ...
                torch.distributed.all_gather_object(gathered, val, group=vllm_ps.get_tensor_model_parallel_group().device_group)
