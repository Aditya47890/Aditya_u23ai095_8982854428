...
from verl.trainer.ppo.analysis_logging import (
    collect_analysis_metrics,
    dataset_kwargs_from_config,
    maybe_write_rollout_artifacts,
    use_virtual_dataset,
)
...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        use_virtual_dataset_flag = use_virtual_dataset(self.config)
        dataset_kwargs = dataset_kwargs_from_config(self.config)

        if use_virtual_dataset_flag:
            # Use virtual dataset, not dependent on actual files
            from verl.utils.dataset.rl_dataset import VirtualRLHFDataset
            
            # Calculate virtual dataset size: based on total training steps and batch size
        ...
        if use_virtual_dataset_flag:
            # Validation dataset also uses virtual mode, but is usually smaller
            val_virtual_size = self.config.data.get('val_virtual_dataset_size', 
                                                  (self.config.data.get('val_batch_size', 32) or 32) * 10)  # Default 10 batches for validation
    ...
    def _validate(self):
        ...
        for test_data in tqdm(self.val_dataloader, desc="Validation"):
            ...
            if self.config.actor_rollout_ref.rollout.name in ['vllm_multi_turn', 'vllm_multi_turn_via_chat']:
                print("Using multi-turn dialogue rollout")
            ...
            for batch_dict in tqdm(self.train_dataloader, desc=f"Epoch {epoch + 1} / {self.config.trainer.total_epochs}", initial=tqdm_initial):
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('reward', timing_raw):
                        ...
                        maybe_write_rollout_artifacts(self.config, self.global_steps, gen_batch_output, batch=batch)
                        metrics.update(collect_analysis_metrics(batch))
