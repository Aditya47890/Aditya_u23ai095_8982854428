...
def qwen_math_equal_subprocess(prediction, reference,  timeout_seconds=10):
    ...
    p.join(timeout=timeout_seconds)  # Wait for process to complete, up to timeout_seconds
    ...
    # If the process completed normally, get the result
    try:
        return q.get_nowait()
    except:
        return False   
...
def preprocess_box_response_for_qwen_prompt(model_output, answer):
    ...
    # # Use re.DOTALL to ensure cross-line text matching
