...
class DataProto:
    ...
    def pop(self, batch_keys=None, non_tensor_batch_keys=None, meta_info_keys=None) -> 'DataProto':
        ...
        for key in non_tensor_batch_keys:
            # print("key",key)#raw_prompt
            # print("self.non_tensor_batch.keys()",self.non_tensor_batch.keys())#self.non_tensor_batch.keys() dict_keys(['data_source', 'ability', 'reward_model', 'extra_info', 'index']) 
    ...
    def custom_repeat_by_counts(self, repeat_counts, interleave=True):
        """Repeat each batch row by its own count."""

        repeat_counts = [int(count) for count in repeat_counts]
        assert len(repeat_counts) == len(self), "repeat_counts must align with batch length"

        if self.batch is not None:
            repeated_tensors = {}
            for key, tensor in self.batch.items():
                pieces = []
                for row_index, count in enumerate(repeat_counts):
                    if count <= 0:
                        continue
                    row = tensor[row_index:row_index + 1]
                    if interleave:
                        pieces.append(row.repeat_interleave(count, dim=0))
                    else:
                        pieces.append(row.expand(count, *row.shape[1:]))
                repeated_tensors[key] = torch.cat(pieces, dim=0) if pieces else tensor[:0]
        ...
        for key, val in self.non_tensor_batch.items():
            ...
            for row_index, count in enumerate(repeat_counts):
                if count <= 0:
                    continue
                row = val[row_index:row_index + 1]
                if interleave:
                    pieces.append(np.repeat(row, count, axis=0))
                else:
                    pieces.append(np.tile(row, (count,) + (1,) * (row.ndim - 1)))
            repeated_non_tensor_batch[key] = np.concatenate(pieces, axis=0) if pieces else val[:0]
