...
#   Role A: Adversarial user simulator (SAGE)
#   Role B: Independent emotion judge (for ECS)
SIMULATOR_MODEL_ID = "mistralai/Mistral-7B-Instruct-v0.3"

# Policy Checkpoints (all 4 models in one run for paired comparison)
POLICY_MODELS = [
    ("Qwen/Qwen2.5-1.5B-Instruct", "Qwen2.5-1.5B-Base"),
    ("Qwen/Qwen2.5-7B-Instruct",   "Qwen2.5-7B-Base"),
...
CHECKPOINT_THINKING_MAP = {
    ...
    "Qwen2.5-7B-Base":   ("Qwen/Qwen2.5-7B-Instruct",   "Qwen/Qwen2.5-7B-Instruct"),
...
RESULTS_FILE = "./results_unified/all_results_rolling.json"
...
for ckpt_id, ckpt_name, ckpt_think in CHECKPOINTS:
    print(f"  {ckpt_name:35s}  thinking={ckpt_think}  [{ckpt_id}]")
print()
...
for _, model_name in POLICY_MODELS:
    for has_thinking in THINKING_MODES:
        ...
        for trajectory in ADVERSARIAL_TRAJECTORIES:
            ...
            print(f"\n  [{traj_id}] — {len(pending)} remaining: {pending}")
...
import zipfile
zip_path = f"{OUTPUT_DIR}/rlver_unified_{ts}.zip"
