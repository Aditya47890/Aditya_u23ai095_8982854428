...
# Single rolling results file — resume reads this
RESULTS_FILE = "./results/all_results_rolling.json"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# ─────────────────────────────────────────────
# RESUME LOGIC: load previously completed work
# ─────────────────────────────────────────────
def load_existing_results():
    """Load existing results and build a set of completed (checkpoint, traj_type, dialogue_index) keys."""
...
def save_rolling(results_list):
    """Overwrite the rolling results file atomically."""
...
for _, model_name in POLICY_MODELS:
    ...
    for has_thinking in THINKING_MODES:
        ...
        # Check if ALL dialogues for this checkpoint are done
        ...
        for trajectory in ADVERSARIAL_TRAJECTORIES:
            ...
            # Which dialogue indices still need running?
            ...
            for d_idx in tqdm(pending, desc=traj_id):
                try:
                    ...
                    save_rolling(all_results)   # atomic save after every dialogue
...
# ─────────────────────────────────────────────────────────────
# ANALYSIS (runs on whatever is in all_results)
# ─────────────────────────────────────────────────────────────
rows = []
